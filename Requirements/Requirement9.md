...
2. mount-observer converts to `AttrPattern[]`
3. assign-gingerly reads attributes using `AttrPattern[]`
4. mount-observer sets up `MutationObserver`

Potential optimization:
- Cache `AttrPattern[]` conversion results
...
### Optimization 1: Cache AttrPattern Conversion
```typescript
const AttrPatternCache = new WeakMap<MountInit, AttrPattern[]>();

function convertToAttrPatterns(
  mountInit: MountInit,
  isCustomElement: boolean
): AttrPattern[] {
  // Check cache first
  if (AttrPatternCache.has(mountInit)) {
    return AttrPatternCache.get(mountInit)!;
  }
  
  // Build and cache
  const mappings = buildAttrPatterns(mountInit.whereAttr, mountInit.map, isCustomElement);
  AttrPatternCache.set(mountInit, mappings);
  return mappings;
}
```
...
function convertToAttrPatterns(...): AttrPattern[] {
  // ...
  mappings.push({
    attrName,
...
if (DEBUG) {
  console.log('[assign-gingerly] Reading attributes:', AttrPatterns);
