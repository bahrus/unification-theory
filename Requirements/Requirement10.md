...
### 9. Invalid AttrPattern
```typescript
const AttrPatterns: AttrPattern[] = [
...
function convertToAttrPatterns(...): AttrPattern[] {
  ...
  for (const [attrName, coordinate] of Object.entries(attrCoordinateMap)) {
    if (!attrName) {
      console.warn('Skipping invalid AttrPattern: empty attrName');
      continue;
    }
    
    const mapEntry = map[coordinate];
    if (!mapEntry?.mapsTo) {
      console.warn(`Skipping AttrPattern: no mapsTo for ${attrName}`);
      continue;
    }
...
export function validateRegistryItem(item: IBaseRegistryItem): string[] {
  ...
  if (item.AttrPatterns) {
    item.AttrPatterns.forEach((mapping, index) => {
      if (!mapping.attrName) {
        errors.push(`AttrPatterns[${index}]: missing attrName`);
      }
      if (!mapping.propName) {
        errors.push(`AttrPatterns[${index}]: missing propName`);
      }
    });
  }
...
- "Attributes not reading" → Check AttrPatterns, attribute names
