# Requirement 3: Create Utility to Convert WhereAttr/MapConfig to AttrPattern[]

## Goal
Build a utility function in mount-observer that converts the existing `whereAttr?: WhereAttr` and `map?: MapConfig` from `MountInit` into the simpler `AttrPatterns: AttrPattern[]` format that assign-gingerly understands.

## Background
mount-observer has a sophisticated attribute coordinate system (via `attrCoordinates.ts`) that builds attribute names from prefixes, bases, and branches. The `MapConfig` then maps coordinates to property names and provides parsing logic. We need to flatten this into the simpler `AttrPattern[]` format.
...
```typescript
/**
 * Converts MountInit's whereAttr and map config into AttrPattern[] for assign-gingerly
 ...
 * @returns Array of AttrPattern objects for assign-gingerly
 */
...
1. Call `convertToAttrPatterns()` to generate `AttrPatterns`
2. Add `AttrPatterns` to the registry item before pushing to `assignGingerlyRegistry`
...
const AttrPatterns = convertToAttrPatterns(
  mountInit.whereAttr, 
  mountInit.map, 
