...
## Background
Currently, when spawning a class instance, `ElementEnhancementContainer.get()` only passes existing values from `oElement.enh.myEnhancement` as `initVals`. We need to also read attributes specified in `registryItem.AttrPatterns` and include those values in `initVals`.
...
```typescript
/**
 * Reads initial attribute values from an element based on AttrPatterns
 * @param element - The element to read attributes from
 * @param AttrPatterns - Array of attribute mapping configurations
 * @returns Object with property names as keys and parsed attribute values
 */
function readInitialAttributes(
  element: Element, 
  AttrPatterns: AttrPattern[] | undefined
): Record<string, any> {
  if (!AttrPatterns || AttrPatterns.length === 0) {
    return {};
  }
  
  const result: Record<string, any> = {};
  
  for (const mapping of AttrPatterns) {
...
const attrInitVals = readInitialAttributes(element, registryItem.AttrPatterns);
...
- No breaking changes to existing code (AttrPatterns is optional)
- Clean separation: assign-gingerly reads initial values, mount-observer handles ongoing observation

## Testing Considerations
- Test with no AttrPatterns (should work as before)
- Test with AttrPatterns but no attributes present
- Test with AttrPatterns and attributes present
- Test precedence: enh values should override attribute values
- Test with custom parser functions

## Next Steps
After this, we need to create a bridge between mount-observer's `WhereAttr`/`MapConfig` and assign-gingerly's `AttrPattern[]`.
